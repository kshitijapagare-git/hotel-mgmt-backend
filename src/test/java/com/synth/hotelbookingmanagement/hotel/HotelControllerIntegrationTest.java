...
class HotelControllerIntegrationTest {
    ...
    private String payload() {
        return "{\"name\":\"test-security\",\"address\":\"test-security\",\"city\":\"test-security\",\"country\":\"test-security\",\"starRating\":1,\"phone\":\"+1-555-0100\",\"email\":\"test@example.com\",\"status\":\"ACTIVE\"}";
    }
    ...
    void should_return_200_when_entity_exists_by_id() throws Exception {
        String location = mockMvc.perform(post("/api/v1/hotels")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(payload()))
                .andExpect(status().isCreated())
        ...
        mockMvc.perform(get(location))
                ...
                .andExpect(jsonPath("$.name").value("test-security"))
                .andExpect(jsonPath("$.address").value("test-security"))
                .andExpect(jsonPath("$.city").value("test-security"))
                .andExpect(jsonPath("$.country").value("test-security"))
                .andExpect(jsonPath("$.starRating").value(1))
    ...
    void should_return_404_when_entity_is_not_found_by_id() throws Exception {
        mockMvc.perform(get("/api/v1/hotels/{id}", "00000000-0000-0000-0000-000000000000"))
    ...
    void should_return_400_when_id_format_is_invalid() throws Exception {
        mockMvc.perform(get("/api/v1/hotels/{id}", "not-a-valid-uuid"))
    ...
    void should_return_400_when_create_request_has_blank_string_fields() throws Exception {
        mockMvc.perform(post("/api/v1/hotels")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"\",\"address\":\"\",\"city\":\"\",\"country\":\"\",\"starRating\":0,\"phone\":\"\",\"email\":\"\",\"status\":\"\"}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.status").value(400));
    }
    ...
    void should_return_404_when_updating_non_existent_entity() throws Exception {
        mockMvc.perform(put("/api/v1/hotels/{id}", "00000000-0000-0000-0000-000000000000")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(payload()))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404));
    ...
    void should_return_404_when_deleting_non_existent_entity() throws Exception {
        mockMvc.perform(delete("/api/v1/hotels/{id}", "00000000-0000-0000-0000-000000000000"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404));
    }
